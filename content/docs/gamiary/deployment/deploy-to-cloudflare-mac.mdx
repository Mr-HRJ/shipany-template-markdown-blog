...
### 如果你的电脑是Windows 或者 你不想每次改动后都在本地执行部署命令`pnpm cf:deploy`，那么请看这篇（也适用于Mac）：[项目部署到 Cloudflare Workers（Windows)](/docs/gamiary/deployment/deploy-to-cloudflare-windows)
...
NEXT_PUBLIC_APP_NAME = "Gamiary Template"
...
[如何购买便宜的域名](/docs/gamiary/others/buy-cheap-domain)
